...
def covic_19():
    ...
    for j in range(0,int(test_cases)):
        ...
        sub_sets = []
        for i in range(1,int(num_of_samples)):
            print(samples)
            count = 1
            # sub_sets.append(count)
            if (abs(int(samples[(i-1)]) - int(samples[i])) < 3):
                print(abs(int(samples[i-1]) - int(samples[i])))
                count+=1

        sub_sets.append(count)

        print("{} {}".format(min(sub_sets),max(sub_sets)))
